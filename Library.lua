...
function Oversimplified:CreateWindow(tTxt)
    ...
    function WO:CreateTab(tN)
        ...
        function E:CreateColorPicker(tx,dc,cb)
            ...
            local opn=false PB.MouseButton1Click:Connect(function() opn=not opn TS:Create(C,TweenInfo.new(0.2),{Size=opn and UDim2.new(1,-14,0,145) or UDim2.new(1,-14,0,34)}):Play() end)
            ...
            local CB=Instance.new("TextButton",SF) CB.Size=UDim2.new(1,0,0,20) CB.BackgroundColor3=Oversimplified.Theme.DarkerBg CB.TextColor3=Oversimplified.Theme.Text CB.Font=Enum.Font.GothamMedium CB.TextSize=11 CB.Text="Chroma: OFF" CB.AutoButtonColor=false Instance.new("UICorner",CB).CornerRadius=UDim.new(0,4) Instance.new("UIStroke",CB).Color=Oversimplified.Theme.Border CB.UIStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
            local cOn, cConn = false, nil
            CB.MouseButton1Click:Connect(function()
                cOn = not cOn
                CB.Text = "Chroma: "..(cOn and "ON" or "OFF")
                CB.TextColor3 = cOn and Oversimplified.Theme.Active or Oversimplified.Theme.Text
                if cOn then
                    cConn = game:GetService("RunService").RenderStepped:Connect(function()
                        local clr = Color3.fromHSV(tick() % 5 / 5, 1, 1)
                        sUpds["R"](clr.R*255, true) sUpds["G"](clr.G*255, true) sUpds["B"](clr.B*255, true)
                        uC()
                    end)
                else
